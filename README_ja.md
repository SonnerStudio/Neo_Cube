...
私たちは現在、究極の進化である **SuperCube-Mode 64** を開発しています。この次世代 64 ビット エミュレーション レイヤーは、完全な仮想ハードウェアの再設計 (Neo-Gekko-64 CPU、Neo-Flipper-64 GPU、Neo-Macaron-64 Audio) を特徴としています。

クラシックなゲームキューブのゲームは、ヒューリスティックなハッシュ化とオンザフライのアセットインジェクションによって同時に変換され、静的なラスタライゼーションを動的なレイトレーシング (グローバルイルミネーションとPBRマテリアル) に置き換えます。システムは、AIアップスケーリング (DLSS/MetalFX) によって 1080p から超高精細な 8K 解像度まで内部的にスケーリングします。

同時に、新しいSuperCube SDKは自作ソフトコミュニティに全く新しい次元を開きます。ノスタルジックで広く愛されているゲームキューブのスタイルに忠実でありながら、無制限のメモリとコンピュートシェーダーを備えた次世代タイトルを開発できます！空間オーディオ (オーディオファンの 432 Hz チューニングを含む) と NVMe PCIe 4.0 エミュレーションによるロード時間ゼロに備えてください。*詳細は近日公開...*
